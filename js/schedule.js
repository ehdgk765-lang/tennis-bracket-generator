...
const Schedule = {
      ...
      <div>
        ...
        <div class="space-y-4 mb-6">
          ${tournament.isCustom ? this._renderCourtLayout(tournament) : (() => {
            ...
            const warmupHtml = _warmupMins > 0 && tournament.startTime ? `
            ...
            return warmupHtml + tournament.timeSlots.map((slot, si) => {
              const courtMap = {};
              ...
              <div class="schedule-slot" data-slot="${si}">
                <div class="flex items-center gap-2 mb-2">
                  ...
                  <span class="text-sm font-bold text-gray-700 bg-gray-100 px-3 py-1 rounded-full">${_gameMins > 0 ? `${slot.time} ~ ${Schedule._addMinutes(slot.time, _gameMins)}` : slot.time}</span>
  ...
  _drawBracketImage(tournament) {
    ...
    const rosterH = rosterRows > 0 ? 28 + rosterRows * rosterCellH + 10 : 0;
    ...
    const matchCellH = nameCellH * 2 + (hasAnyScore ? scoreH : 0) + 20;
    ...
    headerGrad.addColorStop(0, '#0b6b32');
    headerGrad.addColorStop(0.5, '#15803d');
    ...
    if (rosterH > 0) {
      ...
      sortedPlayers.forEach((name, i) => {
        ...
        const cx = PAD + 14 + col * rosterCellW;
    ...
    slots.forEach((slot, si) => {
      ...
      for (let ci = 0; ci < courtCount; ci++) {
        ...
        if (isSingles) {
          ...
          drawNameCell(t1Names[0] || '?', t1X, cellY, teamW, nameCellH, 'right');
          drawNameCell(t1Names[1] || '?', t1X, cellY + nameCellH, teamW, nameCellH, 'right');
          const t2Names = (match.player2 || '?').split(' / ');
          drawNameCell(t2Names[0] || '?', t2X, cellY, teamW, nameCellH, 'left');
          drawNameCell(t2Names[1] || '?', t2X, cellY + nameCellH, teamW, nameCellH, 'left');
        }
    ...
    ctx.fillText('HTL Tennis', W / 2, fY + 20);
    ctx.textAlign = 'left';
